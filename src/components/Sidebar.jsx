...
function Sidebar({
  ...
  const hasActiveFilters =
    minRating > 0 || Boolean(priceFilter) || Boolean(distanceFilter) || Boolean(cuisineFilter);
              ...
              {sortedRestaurants.map((r) => {
                ...
                return (
                  <li key={r.place_id} className="sidebar-item">
                    <button type="button" className="sidebar-item-btn" onClick={() => onRestaurantSelect(r)}>
                      <span className="sidebar-item-name">
                        {isFav && (
                          <span className="sidebar-item-favorite" aria-label="Saved">
                            ★{" "}
                          </span>
                        )}
                        {r.name}
                      ...
                      <div className="sidebar-item-meta">
                        {r.rating && <span className="sidebar-item-rating">★ {r.rating}</span>}
                        {distanceLabel && (
                          <span className="sidebar-item-distance" data-distance-miles={r.distanceMiles.toFixed(3)}>
                            {distanceLabel}
                          </span>
                        )}
                      </div>

                      {r.vicinity && <span className="sidebar-item-address">{r.vicinity}</span>}
